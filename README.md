...
- 对方当事人：创建/更新案件时记录对方姓名与证件号，案件列表与详情中展示。
- 律师利益冲突预检：分配主办及协作律师（含创建案件、更新协作律师）时，若任一律师正在其他**未结案件**中代理证件号相同的对方当事人，整次拒绝（HTTP 409 / 错误码 40903），响应中列出冲突律师与冲突案号，**原分配保持不变**；管理员同样不能绕过。对方证件号缺失时只提示（warning）不阻断；调整律师或对方证件号后可直接重试。
