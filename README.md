...
| POST | /api/v1/cases/:id/assign | 分配主办/协作律师（利益冲突预检，冲突返回 40903 并列出冲突案号，管理员也不可绕过） |
...
- 对方当事人信息：创建/更新案件时记录对方姓名与证件号，案件详情展示；资料缺失只提示不阻断。
- 律师利益冲突预检：分配主办及协作律师时，任一律师在其他未结案件（非 closed/archived）中代理的当事人证件号与本案对方证件号相同，则整次拒绝（HTTP 409 / code=40903）并列出冲突案号，原分配保持不变；该预检对所有角色生效，管理员也不能绕过；调整律师或补录对方信息后可重试。
